<?php
    ...
    class Datagrade {
        ...
        function getsubject(){
            ...
            $r = mysql_query($q);
            $q3 = "select * from studentsubjectlec where studid=$id";
            $r3 = mysql_query($q3);
            $q4 = "select * from studentsubjectlab where studid=$id";
            $r4 = mysql_query($q4);
            $data = array();
        ...
        function getsubjectSY($sy,$classid,$id){
            $q = "select * from class where teacher=$id and classid=$classid and sem='$sy' order by subject asc";
            $r = mysql_query($q);
            return $r;
        }
        ...
        function getgrade($teachid){
            $studid = $this->getid();
            $q = "SELECT * FROM `studentsubjectlec` INNER JOIN criteria ON studentsubjectlec.classid = criteria.classid WHERE studentsubjectlec.studid='$studid' and criteria.teachid='$teachid'";
            $r = mysql_query($q);
        ...
        function getgradelab($teachid){
            $studid = $this->getid();
            

            $q = "SELECT * FROM `studentsubjectlab` INNER JOIN criteria ON studentsubjectlab.classid = criteria.classid WHERE studentsubjectlab.studid='$studid' and criteria.teachid='$teachid'";
            $r = mysql_query($q);
            if($row = mysql_fetch_array($r)){
        ...
        function getgradeleclab($teachid){
            
            $studid = $this->getid();

            $q = "SELECT * FROM `studentsubject` INNER JOIN criteria ON studentsubject.classid = criteria.classid WHERE studentsubject.studid='$studid' and criteria.teachid='$teachid'";
            $r = mysql_query($q);
            if($row = mysql_fetch_array($r)){
